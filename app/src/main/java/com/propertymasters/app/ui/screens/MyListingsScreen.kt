...
@Composable
fun MyListingsScreen(onBack: () -> Unit, onPropertyClick: (Int) -> Unit) {
    var properties by remember { mutableStateOf<List<Property>>(emptyList()) }
    var isLoading by remember { mutableStateOf(true) }

    LaunchedEffect(Unit) {
        properties = FirebaseRepository.getMyProperties()
        isLoading = false
    }
    ...
    Column(
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Text("My Listings", fontWeight = FontWeight.Bold, fontSize = 20.sp, color = TextPrimary)
        }
